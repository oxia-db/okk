...
class ReportTools:
    def __init__(self, config: Config):
        ...
        self._daily_issue_cache: dict[str, int] = {}

    def get_or_create_daily_issue(self) -> str:
        """Get today's tracking issue or create one."""
        today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
        title = f"🤖 okk-pilot daily report — {today}"
        ...
        try:
            body = f"# Daily Verification Report — {today}\n\nEvents and observations from okk-pilot.\n\n---\n🤖 Managed by okk-pilot"
            label_objects = []
    ...
    def close_issue(self, issue_number: int) -> str:
        ...
        try:
            ...
            return json.dumps({"error": f"Failed to close issue #{issue_number}: {e}"})
    ...
    def create_pull_request(self, branch: str, title: str, body: str, files: dict[str, str]) -> str:
        try:
            if not branch.startswith("pilot/"):
                branch = f"pilot/{branch}"

            if "\U0001f916" not in body:
                body += "\n\n---\n\U0001f916 Generated by okk-pilot"
            ...
            for path, content in files.items():
                try:
                    ...
                    self._repo.update_file(
                        path=path, message=f"pilot: update {path}",
                        content=content, sha=existing.sha, branch=branch,
                    )
                except GithubException:
                    self._repo.create_file(
                        path=path, message=f"pilot: add {path}",
            ...
            try:
                pr.add_to_labels("pilot-improvement")
            except GithubException:
                self._repo.create_label(name="pilot-improvement", color="1d76db")
                pr.add_to_labels("pilot-improvement")
